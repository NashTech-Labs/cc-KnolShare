...
class KnolxSessionServiceSpec extends PlaySpecification with Mockito {
  ...
  "KnolxSessionServiceSpec" should {
    ...
    "be able to createKnolxSession : Failure Case" in {
      val knolSession: KnolxSession = KnolxSession(1,"Shivangi",Some("abc"),Some(2),None,java.sql.Date.valueOf("2017-09-12"))
      mockedKnolxSessionDBService.createKnolxSession(knolSession) returns Future.failed(InsertionError("Unable to insert"))
      await(TestObject.createKnolxSession(knolSession)) must throwA[InsertionError]
    }
    ...
    "be able to get all knolx sessions: Failure Case" in {
      val knolSession: KnolxSession = KnolxSession(1,"Shivangi",Some("abc"),Some(2),None,java.sql.Date.valueOf("2017-09-12"))
      mockedKnolxSessionDBService.getAllKnolxSessions() returns Future.failed(DatabaseException("Unable to talk to DB"))
      await(TestObject.getAllKnolxSession()) must throwA[DatabaseException]
    }
