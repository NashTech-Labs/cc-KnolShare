...
class UserComponentSpec extends PlaySpecification with UserComponent with TestDBHelper {
  ...
  "User Component" should {
    ...
    "be able to create new user" in {
      val user = await(createUser(User(Some(2), "userName", "userEmail", "password", "9876543210")))
      user.id.getOrElse(0) must be equalTo 2
    }

    "be able to update user" in {
      val updateCount = await(updateUserById(1,
        User(Some(1), "newUser", "email", "passw", "9876543210")))
