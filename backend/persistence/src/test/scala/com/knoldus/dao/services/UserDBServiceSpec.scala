...
class UserDBServiceSpec extends PlaySpecification with Mockito {
  ...
  "User Service" should {
    ...
    "be able to get user by email" in {
      ...
      Future.successful(Some(User(Some(1), "name", "email", "password", "123456")))
      val user = await(TestDBService.getUserByEmail("sang@gmail.com"))
      user.get.id must beEqualTo(Some(1))
    ...
    "be able to create user" in {
      mockedUserComponent.createUser(User(Some(1), "name", "email", "password", "123456")) returns
      Future.successful(User(Some(1), "name", "email", "password", "123456"))
      val res = await(TestDBService.createUser(User(Some(1), "name", "email", "password", "123456")))
      res.id must beEqualTo(Some(1))
